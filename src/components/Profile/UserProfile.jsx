...
const UserProfile = ({ onClose, onSave }) => {
    ...
    // Valeurs initiales depuis le localStorage
    ...
    // Action à la sauvegarde
    const handleSubmit = (values) => {
        localStorage.setItem('userProfile', JSON.stringify(values));
        // On renvoie le nouveau nom complet au parent (Sidebar) pour qu'il s'actualise
        onSave(`${values.firstName} ${values.lastName}`);
    };

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content bg-dark text-white p-4 rounded-4 shadow" onClick={(e) => e.stopPropagation()}>
                ...
                {/* Zone Avatar (Statique pour l'instant comme sur le design) */}
                <div className="text-center mb-5">
                    <div className="vortex-avatar-container mb-3">
                        <div className="vortex-avatar-circle bg-white"></div>
                        <button className="vortex-edit-avatar-btn">
                            <i className="bi bi-pencil-fill"></i>
                        </button>
                    </div>
                    <p className="vortex-avatar-text mb-0">Changer la photo de profil</p>
                    <p className="text-secondary small">Format JPG, PNG ou WebP. Max 2Mo.</p>
                </div>
                            ...
                            <div>
                                ...
                                <div className="vortex-info-box d-flex gap-3 align-items-start p-3 rounded-3">
                                    ...
                                    <p className="small text-secondary mb-0">
                                        La modification de votre adresse email nécessitera une nouvelle validation par code de sécurité envoyé à votre ancienne adresse.
                                    </p>
                                </div>
                            </div>
